...
/**
 * Get the authorization header for Server Components
 * SERVER-SIDE SAFE - No localStorage
 */
export const getServerAuthHeader = (): string => {
  // Use admin credentials for server-side API calls
  if (config.auth.username && config.auth.appPassword) {
    return `Basic ${config.auth.basicAuth}`;
  }
  return "";
};
